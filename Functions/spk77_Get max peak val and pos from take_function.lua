...
-- EXAMPLE --
--  show peak value/pos and move edit cursor to peak sample
--  insert a stretch marker to take peak sample
...
function main()
  ...
  if not ret then -- something went wrong -> return
    return
  end
  
  msg("\nPeak value: " .. max_peak_val)
  msg("Peak position in item time: " .. peak_sample_pos)
  msg("Peak position in project time: " .. item_pos + peak_sample_pos)
  -- Move edit cursor to peak sample position
  reaper.SetEditCurPos(item_pos + peak_sample_pos, true, false)
  
  
  ---[[ -- Insert stretch marker to take peak sample
  local num_stretch_markers = reaper.GetTakeNumStretchMarkers(take)
  for i=1, num_stretch_markers do
    reaper.DeleteTakeStretchMarkers(take, i-1)
  end
  reaper.SetTakeStretchMarker(take, -1, peak_sample_pos)
  reaper.UpdateArrange()
  --]]
end
