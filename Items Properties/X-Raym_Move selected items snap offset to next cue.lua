--[[
 * ReaScript Name: Move selected items snap offset to next cue
...
function main() -- local (i, j, item, take, track)
	...
	for i = 1, #init_sel_items do
		...
		reaper.Main_OnCommand(40741, 0) -- move edit cursor to previous cue in items
		...
		if cursor_pos ~= old_snap then
			...
			reaper.SetMediaItemInfo_Value(item, "D_SNAPOFFSET", reaper.GetMediaItemInfo_Value(item, "D_LENGTH")) -- Set the 
		end	
	...
	reaper.Undo_EndBlock("Move selected items snap offset to next cue", -1) -- End of the undo block. Leave it at the bottom of your main function.
