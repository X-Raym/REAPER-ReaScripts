--[[
 * ReaScript Name: Toggle selected active takes reverse arround their snap offset
...
function main() -- local (i, j, item, take, track)
	...
	for i = 0, selected_items_count-1  do
		...
		if take ~= nil then -- if ==, it will work on "empty"/text items only
			...
			-- GET INFOS
			old_snap = reaper.GetMediaItemInfo_Value(item, "D_SNAPOFFSET")
			abs_snap = reaper.GetMediaItemInfo_Value(item, "D_POSITION") + old_snap
			new_snap =  reaper.GetMediaItemInfo_Value(item, "D_LENGTH") - old_snap
			...
			reaper.SetMediaItemInfo_Value(item, "D_POSITION", abs_snap - new_snap)

		end -- ENDIF active take
	
	end -- ENDLOOP through selected items

	reaper.Undo_EndBlock("Toggle selected active takes reverse arround their snap offset", -1) -- End of the undo block. Leave it at the bottom of your main function.
