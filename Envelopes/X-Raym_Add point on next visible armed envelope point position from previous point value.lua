--[[
 * ReaScript Name: Add point on next visible armed envelope point position from previous point value
 * Description: Take previous (from cursor position) point value, and insert a new point with that value under the next point. Works on selected tracks, with visble and armed envelope.
				...
				if x ~= nil and w ~= nil then
					...
					if cursor_point ~= -1 then
						...
						if retval3 == true then
							reaper.SetEnvelopePoint(env, cursor_point+1, timeInOptional, valueOut2, shapeInOptional, tensionInOptional, true, false)
						
							-- SET CURSOR POINT
							reaper.InsertEnvelopePoint(env, timeOut3, valueOut3, 0, 0, false, true)
