...
function main() 
	...
	if take ~= nil then
		...
		if start_note < cursor_ppq_pos and mouse_note ~= nil then -- uf edit cursor is after mouse
		
			take_fng = reaper.FNG_AllocMidiTake(take)
			take_fng_count_notes = reaper.FNG_CountMidiNotes(take_fng)
			
			-- offset
			offset = math.floor(cursor_ppq_pos + 0.5 - end_note)
			--msg(end_note)
			--msg(mouse_ppq_pos)
			--msg(offset)
	...
	reaper.Undo_OnStateChange("Trim MIDI note under mouse end to edit cursor and ripple edit")
