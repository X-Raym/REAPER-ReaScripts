 ...
 * Changelog:
 * v2.0 (2024-11-05)
  # Lua version, deprecation of X-Raym_Select-Unselect all MIDI notes with same pitch right of the mouse.eel
...
local reaper = reaper
local undo_text = "Select-Unselect all MIDI notes with same pitch right of the mouse"
    ...
    if take then
      ...
      for noteidx = notes -1, 0, -1 do
        ...
        if reaper.MIDI_GetProjTimeFromPPQPos( take, startppqpos ) >= cursor_time then
          if noteRow == pitch and not selected then
            select_all = true
            break
          end
        else
...
Main()
reaper.PreventUIRefresh(-1)
reaper.Undo_OnStateChange("Select-Unselect all MIDI notes with same pitch right of the mouse")
