 ...
 * MetaPackage: true
 * Provides:
 *   [main] . > X-Raym_Click at X Y screen position relative to focused window_slot 1.lua
 *   [main] . > X-Raym_Click at X Y screen position relative to focused window_slot 2.lua
 *   [main] . > X-Raym_Click at X Y screen position relative to focused window_slot 3.lua
 *   [main] . > X-Raym_Click at X Y screen position relative to focused window_slot 4.lua
--]]
...
function Init()
  ...
  --hwnd_title = reaper.GetExtState(ext_name, "hwnd_title" .. slot, false)
  if x ~= "" and y ~= "" then -- and hwnd_title ~= "" then
    x = tonumber( x )
    y = tonumber( y )
    if x and y then
      local mouse_x, mouse_y = reaper.GetMousePosition()
      hwnd = reaper.JS_Window_FromPoint( mouse_x, mouse_y )
      if not hwnd then return false end
      x, y = reaper.JS_Window_ClientToScreen( hwnd, x, y )
